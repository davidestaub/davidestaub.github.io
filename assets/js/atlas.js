...
try { const response=await fetch('../assets/data/atlas-catalogue.json?v=reader3'); if(!response.ok)throw new Error('Catalogue download failed'); const data=await response.json();catalogue=data.planets;systems=data.systems; }
...
try{const r=await fetch('../assets/data/atlas-galaxy.json?v=reader3');if(r.ok)galaxy=AtlasGalaxy(await r.json(),catalogue);}catch(e){$('#galaxy-readout').textContent='The map data could not load. Please refresh.';}
...
let simDays=0,orbitLast=0,orbitZoom=1,orbitRunning=!matchMedia('(prefers-reduced-motion: reduce)').matches,timelineDays=1;
function systemTable(p){const sys=systems[p.host];return `<div class="parameter-scroll"><table class="system-table"><thead><tr><th>Planet</th><th>Orbit size</th><th>Year</th><th>Source</th></tr></thead><tbody>${sys.orbits.map(d=>`<tr><td>${esc(d.name)}</td><td>${fmt(d.a)} AU</td><td>${fmt(d.period)} days</td><td>${d.sources.map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.title)}</a>`).join('<br>')}${d.derivation?`<p>${esc(d.derivation)}</p>`:''}${d.eccentricityAssumed?'<p>Circular path assumed.</p>':''}</td></tr>`).join('')}</tbody></table></div>`;}
function renderSystem(p){
 const sys=systems[p.host];simDays=0;timelineDays=Math.max(...sys.orbits.map(d=>d.period))*2;
 $('#orbit-map').innerHTML=sys.orbits.map((d,i)=>`<ellipse id="path-${i}" cy="165" fill="none" stroke="${d.name===p.archiveName?'#ffd580':'#796341'}" stroke-width="1"/><g id="body-${i}" data-orbit-name="${esc(d.name)}"><circle r="${d.name===p.archiveName?4:3}" fill="${d.name===p.archiveName?'#ffdb91':'#c6a374'}"/><text x="7" y="-7">${esc(d.label)}</text></g>`).join('')+(sys.stellarOrbit?'<g id="star-a"><circle r="4" fill="#ffe0aa"/><text x="6" y="-6">A</text></g><g id="star-b"><circle r="3" fill="#ffc38e"/><text x="6" y="-6">B</text></g>':'');
 $('.orbital-chart #orbit-map + circle').style.display=sys.stellarOrbit?'none':'';
 const note=$('#panel-system > .companion-note');note.hidden=!sys.companionText;note.innerHTML=`<summary>Other stars</summary><p>${esc(sys.companionText)}</p>`;
 $('#system-planets')?.remove();$('#panel-system').insertAdjacentHTML('beforeend',`<details id="system-planets" class="companion-note"><summary>Orbits and sources</summary><p>${esc(sys.model)}</p>${systemTable(p)}${sys.stellarOrbit?`<p>The two stars orbit their shared centre of mass every ${fmt(sys.stellarOrbit.period)} days. <a href="${sys.stellarOrbit.source.url}" target="_blank" rel="noopener">${esc(sys.stellarOrbit.source.title)}</a></p>`:''}</details>`);
 $('#time-range').textContent='0–'+fmt(timelineDays/365.25)+' years';$('#system-time').value=0;
 $('#motion-note').textContent=orbitRunning?(p.year>365.25?'This planet takes '+fmt(p.year/365.25)+' years to orbit. Move the timeline to explore its path.':''): 'Automatic motion is off because your device requests reduced motion. The timeline still moves the planets.';
 orbitZoom=1;$('#system-zoom').value=0;drawOrbits();
}
const position=AtlasMath.position;
function drawOrbits(){if(!current)return;const sys=systems[current.host],max=Math.max(...sys.orbits.map(d=>d.a*(1+d.eccentricity)),.01),scale=145/max*orbitZoom;
 sys.orbits.forEach((d,i)=>{const r=d.a*scale,path=$('#path-'+i);path.setAttribute('cx',245-r*d.eccentricity);path.setAttribute('rx',r);path.setAttribute('ry',r*Math.sqrt(1-d.eccentricity*d.eccentricity));const [x,y]=position(d,simDays+d.period*i*.173);$('#body-'+i).setAttribute('transform',`translate(${245+x*scale} ${165+y*scale})`);});
 if(sys.stellarOrbit){const b=sys.stellarOrbit,[x,y]=position(b,simDays),sum=b.massA+b.massB;$('#star-a').setAttribute('transform',`translate(${245-x*scale*b.massB/sum} ${165-y*scale*b.massB/sum})`);$('#star-b').setAttribute('transform',`translate(${245+x*scale*b.massA/sum} ${165+y*scale*b.massA/sum})`);}
 $('#distance-scale').textContent=fmt(100/scale)+' AU';$('#system-clock').textContent=simDays<365.25?simDays.toFixed(2)+' days':(simDays/365.25).toFixed(3)+' years';$('#system-time').value=(simDays%timelineDays)/timelineDays;
}
...
$('#system-time').addEventListener('input',e=>{simDays=+e.target.value*timelineDays;drawOrbits();});
matchMedia('(prefers-reduced-motion: reduce)').addEventListener('change',e=>{orbitRunning=!e.matches;if(current)$('#motion-note').textContent=e.matches?'Automatic motion is off because your device requests reduced motion. Use the timeline to move the planets.':'';});
function orbitFrame(t){const dt=Math.min((t-orbitLast)/1000,.1);orbitLast=t;if(current&&orbitRunning&&!document.hidden&&!$('#dossier').hidden&&!$('#panel-system').hidden){simDays+=dt*Math.pow(10,+$('#system-speed').value);drawOrbits();}requestAnimationFrame(orbitFrame);}requestAnimationFrame(orbitFrame);
